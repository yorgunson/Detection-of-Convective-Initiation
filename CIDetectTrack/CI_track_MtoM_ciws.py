...
def SAL_Filter(refgrids,refarea,iterobjs,issue,lead,level,bounds,th,db,config,src,out_q):
    ...
    for iterobj in iterobjs:
        ...
        #itergrids=dbutils.get_SingleObjij_Cmass(db,config.obj_table,src,iterobj[0],issue,lead,level,th,itercmass,bounds)
...
def trackCI_Obs(src,db,config,dtime,issue,lead,level,th,bounds,endtimestep,cifield,evolfield,trackfield):
    
    #print '***********************'
    #print 'Tracking CI forward in time'
    
    ci,ci_id=dbutils.get_allCI(db,config.obj_table,src,dtime,th,bounds,cifield)
    ...
    for i in range(len(ci)):
        ...
        ci_evol_list.append([reftime,refcmass])
        		
        # Form the time iteration list
        timelist=[reftime+timedelta(minutes=k*config.timestep) for k in range(1,endtimestep)]
        
        # Reflist / IntermList Elements = 0:datetime, 1:cmass0, 2:cmass1, 3:obj_area, 5:obj_max
        ref_list=[]
        ref_list.append((reftime,refcmass[0],refcmass[1],ci[i][1],ci[i][2]))
        ...
        for time in timelist:
            ...
            for refobj in ref_list:
                ...
                iterobjs=dbutils.get_allObj_MBS(db,config.obj_table,src,time,th,bounds,refcmass,config.MBS_HalfSide)
                ...
                if len(iterobjs)>0:
                    ...
                    ###############################
                    reftime=time-timedelta(minutes=config.timestep)
                    refarea=refobj[3]
                    refgrids=dbutils.get_SingleObjij_Cmass(db,config.obj_table,src,reftime,issue,lead,level,th,refcmass,bounds)
                    
                    # MultiProcessing
                    #######################
                    out_q=Queue()
                    procs=[multiprocessing.Process(target=SAL_Filter, args=(refgrids,refarea,iterobjs,issue,lead,level,bounds,th,db,config,src,out_q))]
        ...
        for k in range(len(ci_evol_list)):
            if (ci_evol_list[k][0],ci_evol_list[k][1][0],ci_evol_list[k][1][1]) not in checklist:
                dbutils.write_CIevol_Cmass(db,config.obj_table,src,ci_evol_list[k][0],issue,lead,level,th,ci_evol_list[k][1],bounds,evolfield,trackfield,ci_id[i])
